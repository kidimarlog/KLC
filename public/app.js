...
async function klcSetPantsStatus(itemId,status,pickedModel=""){
  try{
    await api("/api/item/status_pants_safe",{
      method:"POST",
      body:JSON.stringify({itemId,status,pickedModel})
    });
    await refresh();
  }catch(e){
...
function klcPickPantsItem(itemId){
  ...
  const val = String(el?.value || "").trim();
  if(!/^\d{6}$/.test(val)){
    alert("מספר הספרות בדגם אינו תואם");
    if(el) el.focus();
    return;
  }
  klcSetPantsStatus(itemId,"picked",val);
}

function klcPantsActions(i){
  return `
    <div class="actions pants-actions-final">
      <button class="green" onclick="klcPickPantsItem('${i.id}')">לוקט</button>
      <button class="red" onclick="klcSetPantsStatus('${i.id}','not_found',document.getElementById('picked_model_${i.id}')?.value || '')">אין מלאי</button>
      <button class="gray" onclick="klcSetPantsStatus('${i.id}','open')">בטל סימון</button>
    </div>
  `;
}
...
function klcPantsModelInput(i){
  return `
    <div class="pants-entry-line">
      <span class="pants-entry-label">הזן דגם</span>
      <input id="picked_model_${i.id}" type="tel" inputmode="numeric" maxlength="6"
        value="${esc(i.picked_model || "")}" placeholder="6 ספרות"
        class="pants-entry-input"
...
function renderWorkerPage(){
  ...
  workerItems.innerHTML = table(
    ...
    sortedItems.map(i=>[
      esc(i.location || "ללא מיקום"),
      isPants ? `<div class="pants-model-clean"><div class="pants-required-clean">${esc(i.model)}</div>${klcPantsModelInput(i)}</div>` : esc(i.model),
      esc(i.mix || "A"),
      esc(i.qty || 1),
      `${statusLabel(i.status)}${i.picked_model?`<br><span class="small">דגם שלוקט: ${esc(i.picked_model)}</span>`:""}`,
      isPants ? klcPantsActions(i) : klcRegularActions(i)
    ]),
    sortedItems.map(i=>i.status)
  );
